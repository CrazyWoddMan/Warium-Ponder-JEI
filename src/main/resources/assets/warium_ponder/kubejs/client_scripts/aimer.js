Ponder.registry((event) => {
    event.create("crusty_chunks:aimer").scene("aimer", "How to use Weapon Aimer", "kubejs:aimer", (scene, util) => {
        scene.showBasePlate();
		...
		scene.showControls(100, [5, 1, 5], "down").withItem("crusty_chunks:aimer");
		scene.text(100, "Weapon Aimer can be used to aim any placeable weaponary from Warium, including...", [6, 1.5, 8.3]).placeNearTarget().attachKeyFrame();
		
		scene.idle(110);
		
		scene.text(40, "All types of rockets", [2.5, 2.0, 12.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(50);
		
		scene.text(40, "Countermeasure Launcher", [9.5, 1.5, 12.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(50);
		
		scene.text(40, "Mortar", [12.5, 1.5, 12.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(50);
		
		scene.text(40, "Any cannons", [12.5, 1.5, 9.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(50);
		
		scene.text(40, "Smoke Launcher", [12.5, 1, 6.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(50);
		
		scene.text(40, "And Armor Peeler or Fire Spear Rocket Pod", [11.5, 1.5, 3.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(60);
		
		let out = {};
		scene.overlay.showOutline(PonderPalette.BLUE, out, [2, 2, 12], 185);
		scene.showControls(100, [2.5, 2.5, 12.5], "down").rightClick().withItem("crusty_chunks:aimer");
		scene.text(100, "Right-click weapon or it's main part with Weapon Aimer in second hand to attach", [2.5, 2.0, 12.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(120);
		
		let out2 = {};
		scene.overlay.showOutline(PonderPalette.BLUE, out2, [5, 1, 12], 65);
		scene.text(60, "For rockets main part is Ordinance Core", [2.5, 2, 12.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(80);
		
		scene.text(120, "For Rocket Pod it's front part", [10.5, 1.5, 3.5]).placeNearTarget();
		scene.overlay.showOutline(PonderPalette.BLUE, out, [10, 1, 3], 125);
	}).scene("aimer_node", "Aimer Node", "kubejs:aimer", (scene, util) => {
        scene.showBasePlate();
		scene.scaleSceneView(0.5);
		scene.setSceneOffsetY(-2.0);
		scene.world.showIndependentSectionImmediately([2, 1, 12]);
		scene.world.showIndependentSectionImmediately([2, 2, 12]);
		scene.world.showIndependentSectionImmediately([2, 3, 12]);
		scene.world.showIndependentSectionImmediately([5, 1, 11]);
		scene.world.showIndependentSectionImmediately([5, 1, 12]);
		scene.world.showIndependentSectionImmediately([5, 1, 13]);
		scene.world.showIndependentSectionImmediately([9, 1, 12]);
		scene.world.showIndependentSectionImmediately([12, 1, 12]);
		scene.world.showIndependentSectionImmediately([12, 1, 9]);
		scene.world.showIndependentSectionImmediately([11, 1, 9]);
		scene.world.showIndependentSectionImmediately([12, 1, 6]);
		scene.world.showIndependentSectionImmediately([10, 1, 3]);
		scene.world.showIndependentSectionImmediately([11, 1, 3]);
		scene.world.showIndependentSectionImmediately([12, 1, 3]);
		...
		let dispenser2 = {};
		scene.overlay.showOutline(PonderPalette.GREEN, dispenser2, [9, 1, 12], 145);
		let mortar = {};
		scene.overlay.showOutline(PonderPalette.GREEN, mortar, [12, 1, 12], 145);
		let cannon2 = {};
		scene.overlay.showOutline(PonderPalette.GREEN, cannon2, [11, 1, 9, 12, 1, 9], 145);
		let smoke2 = {};
		scene.overlay.showOutline(PonderPalette.GREEN, smoke2, [12, 1, 6], 145);
		let pod = {};
		...
		scene.overlay.showOutline(PonderPalette.BLUE, dispenser2, [9, 1, 12], 125);
		scene.overlay.showOutline(PonderPalette.BLUE, smoke2, [12, 1, 6], 125);
		...
		scene.world.showIndependentSectionImmediately([8, 1, 12]);
		
		scene.idle(5);
		
		scene.world.showIndependentSectionImmediately([12, 1, 8]);
