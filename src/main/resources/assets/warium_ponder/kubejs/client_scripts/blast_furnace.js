Ponder.registry((event) => {
    event.create("crusty_chunks:blast_furnace").scene("blast_furnace", "Alloy Furnace", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:blast_furnace", false);
		let furnace = scene.world.showIndependentSection([2, 1, 2], Facing.DOWN);
		
		scene.idle(20);
		
		scene.text(60, "Alloy Furnace mixes ore dusts into alloyed ingots", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		...
		scene.text(120, "3 Blast Funnels must be ontop", [2.5, 4, 2.5]).placeNearTarget().attachKeyFrame();
