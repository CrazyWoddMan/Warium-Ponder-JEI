Ponder.registry((event) => {
    event.create("crusty_chunks:conveyor").scene("conveyor", "Conveyor", (scene, util) => {
        scene.showBasePlate();
	...
	}).scene("conveyor_dropoff", "Conveyor Dropoff", (scene, util) => {
        scene.showBasePlate();
		scene.setSceneOffsetY(-1);
		...
		scene.text(80, "Conveyor Dropoff works similar to Conveyor", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
	...
	}).scene("conveyor_lift2", "Conveyor Lift", (scene, util) => {
        scene.showBasePlate();
		scene.setSceneOffsetY(-1);
		...
		scene.text(80, "Conveyor lift works similar to Conveyor as well", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
