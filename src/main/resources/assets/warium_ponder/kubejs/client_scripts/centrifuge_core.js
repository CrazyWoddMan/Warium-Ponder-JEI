Ponder.registry((event) => {
    event.create("crusty_chunks:centrifuge_core").scene("centrifuge_core", "Centrifuge Core", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 2, 2], "crusty_chunks:centrifuge_core", false);
		let core = scene.world.showIndependentSection([2, 2, 2], Facing.DOWN);
		scene.world.moveSection(core, [0, -1, 0], 0);
		
		scene.idle(20);
		
		scene.text(80, "Centrifuge Core is the main part of the centrifuge", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		...
		scene.text(60, "Centrifuge Bottom is required below", [2.5, 1.2, 2.5]).placeNearTarget().attachKeyFrame();

		scene.idle(70);
		
		scene.overlay.showOutline(PonderPalette.BLUE, out, [2, 3, 2], 65);
		scene.text(60, "Centrifuge Top from the top", [2.5, 3.5, 2.5]).placeNearTarget().attachKeyFrame();
