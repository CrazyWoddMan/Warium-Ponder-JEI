Ponder.registry((event) => {
    event.create("crusty_chunks:machine_gun").scene("machine_gun", "Machine Gun", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:machine_gun", false);
		...
		scene.world.setBlock([2, 1, 1], "crusty_chunks:machine_gun_barrel", false);
		scene.world.showIndependentSection([2, 1, 1], Facing.SOUTH);
		
		scene.idle(10);
		
		scene.text(60, "Machine Gun Barrel is required to shoot", [2.5, 1.5, 1.5]).placeNearTarget().attachKeyFrame();
