Ponder.registry((event) => {
    event.create("crusty_chunks:engine_cyllinder").scene("engine_cyllinder", "Large Engine Cyllinder", "kubejs:9x9", (scene, util) => {
        scene.showBasePlate();
		scene.setSceneOffsetY(-1);
		
		scene.world.setBlock([4, 2, 4], "crusty_chunks:engine_cyllinder", false);
		let cyllinder = scene.world.showIndependentSection([4, 2, 4], Facing.DOWN);
		scene.world.moveSection(cyllinder, [0, -1, 0], 0);
		
		scene.idle(20);
		
		scene.text(100, "Large Engine Cyllinder is used to build large combustion engine", [4.5, 1.5, 4.5]).placeNearTarget().attachKeyFrame();

		scene.idle(115);

		scene.world.moveSection(cyllinder, [0, 1, 0], 5);
		...
		scene.overlay.showOutline(PonderPalette.BLUE, out, [4, 3, 4], 55);
		scene.text(50, "Large Engine Smokestack", [4.5, 3.2, 4.5]).placeNearTarget().attachKeyFrame();
