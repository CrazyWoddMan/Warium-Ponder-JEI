Ponder.registry((event) => {
    event.create("crusty_chunks:centrifuge_top").scene("centrifuge_top", "Centrifuge Top", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 3, 2], "crusty_chunks:centrifuge_top", false);
		let top = scene.world.showIndependentSection([2, 3, 2], Facing.DOWN);
		scene.world.moveSection(top, [0, -2, 0], 0);
		
		scene.idle(20);
		
		scene.text(100, "Centrifuge Top outputs enriched material from the centrifuge", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		...
		scene.world.showIndependentSection([2, 2, 2], Facing.UP);

		scene.idle(5);

		scene.world.setBlock([2, 1, 2], "crusty_chunks:centrifuge_bottom", false);
		scene.world.showIndependentSection([2, 1, 2], Facing.UP);

		scene.idle(20);

		let out = {};
		scene.overlay.showOutline(PonderPalette.BLUE, out, [2, 2, 2], 65);
		scene.text(60, "Centrifuge Core is required below", [2.5, 2.2, 2.5]).placeNearTarget().attachKeyFrame();

		scene.idle(70);
		
		scene.overlay.showOutline(PonderPalette.BLUE, out, [2, 1, 2], 65);
		scene.text(60, "Centrifuge Bottom below Centrifuge Core", [2.5, 1.2, 2.5]).placeNearTarget().attachKeyFrame();
