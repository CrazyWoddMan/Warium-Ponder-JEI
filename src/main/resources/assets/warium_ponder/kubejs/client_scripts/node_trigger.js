Ponder.registry((event) => {
    event.create("crusty_chunks:node_trigger").scene("node_trigger", "Node Trigger", "kubejs:aimer", (scene, util) => {
        scene.showBasePlate();
		...
		scene.world.showIndependentSection([12, 1, 5], Facing.DOWN);
		
		scene.idle(3);
		
		let trigger4 = scene.world.showIndependentSection([10, 1, 2], Facing.DOWN);
		...
		let pod = {};
		let trigger = {};
		scene.overlay.showOutline(PonderPalette.BLUE, trigger, [10, 1, 2], 275);
		...
		scene.world.moveSection(trigger4, [1, 0, 0], 10);
		
		scene.idle(10);
		
		scene.overlay.showOutline(PonderPalette.RED, trigger, [11, 1, 2], 105);
		...
		scene.overlay.showLine(PonderPalette.GREEN, [2.3, 1.5, 12.5], [7.3, 1.5, 7.1], 145);
