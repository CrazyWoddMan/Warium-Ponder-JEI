Ponder.registry((event) => {
    event.create("crusty_chunks:autocannon_barrel").scene("autocannon_barrel", "Autocannon Barrel", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:autocannon_barrel", false);
		...
		scene.text(80, "Rotary Autocannon Barrel is required for 2 types of Auto Cannons", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(100);
		
		scene.world.moveSection(barrel, [0, 0, -1], 5);
		
		scene.idle(5);
		
		scene.world.setBlock([2, 1, 3], "crusty_chunks:light_autocannon", false);
		let cannon = scene.world.showIndependentSection([2, 1, 3], Facing.NORTH);
		scene.world.moveSection(cannon, [0, 0, -1], 0);
		
		scene.idle(10);
		
		scene.world.moveSection(cannon, [1, 0, 0], 10);
		scene.world.moveSection(barrel, [1, 0, 0], 10);
		
		scene.idle(5);
		
		scene.world.setBlock([1, 1, 2], "crusty_chunks:autocannon", false);
		scene.world.showIndependentSection([1, 1, 2], Facing.DOWN);
		
		scene.idle(5);
		
		scene.world.setBlock([1, 1, 1], "crusty_chunks:autocannon_barrel", false);	
		scene.world.showIndependentSection([1, 1, 1], Facing.SOUTH);
		
		scene.idle(5);
		
		scene.text(40, "Light Auto Cannon", [3.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(60);
		
		scene.text(40, "And Heavy Auto Cannon", [1.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		
		scene.idle(20);
	}).scene("light_autocannon", "Light Auto Cannon", (scene, util) => {
        scene.showBasePlate();
	...
	}).scene("autocannon", "Heavy Auto Cannon", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:autocannon", false);
		...
		scene.showControls(60, [2.5, 2, 2.5], "down").withItem("crusty_chunks:small_shell");
		scene.text(60, "Heavy Auto Cannon shoots Medium Shells", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
		...
		scene.text(80, "To insert shells in Heavy Auto Cannon, Autocannon Drum should be placed on top", [2.5, 2.5, 2.5]).placeNearTarget().attachKeyFrame();
		...
		scene.addKeyframe();
		scene.showControls(30, [2.0, 1.5, 2.5], "left").rightClick().withItem("crusty_chunks:small_shell");
		...
		for (let e = 0; e < 5; e++) {
			...
			scene.particles.simple(1, "flame", [2.2, 2.2, -4]).density(3).area([2.8, 2.8, -5]);
			scene.world.createEntity("crusty_chunks:huge_bullet_fire", [2.5, 2.5, -4.1], e => {
				e.setDeltaMovement([0, 0.3, -2]);
			})
			scene.idle(6);
