Ponder.registry((event) => {
    event.create("crusty_chunks:oil_firebox").scene("oil_firebox", "Kerosene Firebox", "kubejs:7x7", (scene, util) => {
        scene.showBasePlate();
		scene.setSceneOffsetY(-1);
		
		scene.world.setBlock([3, 1, 3], "crusty_chunks:oil_firebox", false);
		scene.world.showSection([3, 1, 3], Facing.DOWN);
		
		scene.idle(20);
		
		scene.text(100, "Kerosene Firebox produces heat from Kerosene and is optional instead of regular Firebox", [3.5, 1.5, 3.5]).placeNearTarget().attachKeyFrame();
		...
		scene.text(60, "Kerosene Firebox can be used for Foundry", [0.5, 2.2, 3],).placeNearTarget().attachKeyFrame();
