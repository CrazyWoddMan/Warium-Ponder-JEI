Ponder.registry((event) => {
    event.create("crusty_chunks:breeder_reactor_port").scene("breeder_reactor_port", "Breeder Reactor Port", "kubejs:7x7", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([3, 1, 3], "crusty_chunks:breeder_reactor_port", false);
		let main = scene.world.showIndependentSection([3, 1, 3], Facing.DOWN);
		
		scene.idle(20);
		
		scene.text(120, "Breeder Reactor Port is used to output items from the breeder reactor", [3.5, 1.5, 3.5]).placeNearTarget().attachKeyFrame();
