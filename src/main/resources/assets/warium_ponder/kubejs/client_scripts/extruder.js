Ponder.registry((event) => {
    event.create("crusty_chunks:extruder").scene("extruder", "Extruder", (scene, util) => {
        scene.showBasePlate();
		scene.setSceneOffsetY(-1);
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:extruder", false);
		scene.world.showSection([2, 1, 2], Facing.DOWN);
		
		scene.idle(20);
		
		scene.text(100, "Extruder has 2 requirements to work", [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
