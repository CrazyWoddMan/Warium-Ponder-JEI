Ponder.registry((event) => {
    event.create("crusty_chunks:ordinance_fission_initiator_head").scene("ordinance_fission_initiator_head", "Ordinance Fission Initiator Head", (scene, util) => {

        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:ordinance_fission_initiator_head", false);
		let head = scene.world.showIndependentSection([2, 1, 2], Facing.DOWN);
		
		scene.idle(10);
		
		scene.text(100, "Ordinance Kinetic Nose is used to build a droppable Fission Bomb", [2.5, 1.5, 2.5]).placeNearTarget();
		...
		scene.world.setBlock([2, 1, 3], "crusty_chunks:ordinance_inline_fission_warhead", false);
		let inline = scene.world.showIndependentSectionImmediately([2, 1, 3]);
		scene.world.moveSection(inline, [0, 0, -2], 5);
		...
		scene.world.moveSection(core, [0, 0, -2], 5);
		...
		scene.text(60, "Ordinance Core Fission Bomb is the main part", [2.5, 1.5, 1.8]).placeNearTarget().attachKeyFrame();
		
		scene.idle(80);
		
		scene.text(175, "It must be filled with 6x Implosion Lens and 1x Fission Core in order to be ready for an activation", [2.0, 1.9, 1.8]).placeNearTarget();
