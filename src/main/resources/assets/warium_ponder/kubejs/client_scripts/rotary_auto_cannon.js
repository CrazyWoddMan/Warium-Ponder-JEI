Ponder.registry((event) => {
    event.create("crusty_chunks:rotary_auto_cannon").scene("rotary_auto_cannon", "Rotary Auto Cannon", (scene, util) => {
        scene.showBasePlate();
		
		scene.world.setBlock([2, 1, 2], "crusty_chunks:rotary_auto_cannon", false);
		...
		scene.world.setBlock([2, 1, 1], "crusty_chunks:rac_barrel", false);
		let barrel = scene.world.showIndependentSection([2, 1, 1], Facing.SOUTH);
		
		scene.idle(10);
		
		scene.text(60, "Rotary Autocannon Barrel is required to shoot", [2.5, 1.5, 1.5]).placeNearTarget().attachKeyFrame();
